...
const checkForMercuryAndSendSMS = async () => {
    ...
    const wasMercuryInRetrogradeYesterdayFunction = async () => {
      ...
      try {
        const response = await fetch(`${mercury_api}?date=${yesterdayFormatted}`, {
          method: 'GET',
        });
        const data = await response.json();
    ...
    const getRandomGif = async () => {
      ...
      try {
        const response = await fetch(`${giphy_api}?api_key=${giphy_api_key}&q=${searchString}&limit=10&offset=0&rating=g&lang=en&bundle=messaging_non_clips`, {
          method: 'GET',
        });
    ...
    const wasMercuryInRetrogradeYesterday = await wasMercuryInRetrogradeYesterdayFunction()
